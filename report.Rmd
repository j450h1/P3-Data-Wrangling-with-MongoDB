...
##INTRODUCTION
I chose this particular area because it is my home city where I was born and raised. This domain knowledge was very helpful as I could clean the data with some context instead of doing guesswork or doing extensive research on another city. 

I chose the GVRD, instead of the City of Vancouver itself, because of the limited data that has been populated so far for Vancouver on OpenStreetMaps and I needed to analyze a dataset larger than 50 MB for this project.
...
###Street name typos
The domain knowledge of the city really helped me here to easily spot typos.
For example, ('ing George Hwy') might not sound odd to a foreigner for a street name, but I realized right away it was supposed to be "King" George and also the highway had actually recently been changed to a Boulevard. So I renamed this correctly to "King George Boulevard". For example, another typo  I fixed was an extra leading space in the street name (' Beatty St'). 
```{r eval=FALSE}
...
```
There were was one street which I originally thought was a typo, but a quick Google search revealed that 'Wynd' is a valid street type. Another interesting 
one was 'Kingsway'; I was familiar with the street name and I always assumed there was an ending such as 'Kingsway Street' that was not widely used. Once again my intuition was incorrect and I learned that Kingsway by itself (one word) was correct.
...
I searched the original dataset and found that there was alreay a 'type' field in a very few number of elements and it was overriding my type setting code which I had near the start of my function.

I adjusted my python cleaning code (5. data.py)) to move the type setting (to node or way) into later on in the code so that it woudln't be overriden. 
...
Field names wi
th double colons were caught by a regular expression I created and were not included in the final JSON file. However, the lower_colon regex did not match 
...
***
Final JSON File -  160 MB - GVRD - Vancouver - OSM XML Raw.osm.json
###Bash
```{r eval=FALSE}
...
-rw-r--r--    1 user     Administ     160M Feb  2 00:09 GVRD - Vancouver - OSM XML Raw.osm.json
```
...
0
> db.van.find( {"created.user" : {$exists: 1} } )
No documents found
...
```{r eval=FALSE}
> db.van.aggregate( [{"$match": {"source": {"$exists" : 1} } }, {"$group": {"_id": "$source", "count": {"$sum":1} } }, 
...
{ "_id" : "Bing", "count" : 7241 }
{ "_id" : "PGS", "count" : 5560 }
{ "_id" : "Yahoo", "count" : 2422 }
{ "_id" : "City of Surrey 2010 Orthographs", "count" : 1831 }
{ "_id" : "survey; Bing", "count" : 1713 }
{ "_id" : "City of Surrey 2011 GIS Data", "count" : 1137 }
{ "_id" : "bing", "count" : 676 }
```
...
{ "_id" : "A&W", "count" : 18 }
```
###Total Number of Nodes or Way with Street Names in Dataset
```{r eval=FALSE}
> db.van.find({"address.street": {"$exists":1}}).count()
84517
```
###Total Number of Unique Street Names
```{r eval=FALSE}
> db.van.distinct("address.street").length
1835
```

As I drive around town I notice some streets are named after one of the 10 Canadian Provinces.
```{r eval=FALSE}
> db.van.distinct("address.street", {"address.street" : {"$in":[/^Ontario/,/^Quebec/,/^British Columbia/,/^Alberta/,/^Sasketchewan/,/^Manitoba/,/^Newfoundland and Labrador/,/^Nova scotia/,/^New Brunswick/,/^Prince Edward Island/]}})
...
A lot of these stats have to be taken with a grain of salt, because it seems a very small percentage of the data are populated with much data other than location info, id, and type which can be seen from visual inspection of the data. 
...
I realized that there could be a difference between nodes and ways so I ran seperate queries.

##Nodes

Here is the result of the query to see the number of node documents 3 fields:
```{r eval=FALSE}
...
After querying MongoDB a few dozen times I realized how beneficial the early cleansing was as it makes it easier when field names and values are standardized into one form (if possible). For example, I used regular expressions to find query 'name' for royal names such as 'King'. If the data was in different fields such as 'store_name', 'restaurant_name', then it would have been very difficult to extract this info from one simple query.

For example, one frustration I stil had was there were still some documents that had different fields with very similar meanings. For example, 'is_in" and 'address.city' where usually similar and if i wanted to query it would be easier if I could be certain all applicable documents had one field. 

Finally, I found it curious that not even one user "tagged" their name or user 
id to get credit for adding any of the nodes or ways to the database as their is no field for created
\pagebreak
#REFERENCES

#Currently taking MongoDB for Python Developers course concurrently
