...
for _, element in ET.iterparse(file_in):
    if element.tag in ["node","way","tag","nd"]:
        ...
        for ndtag in ndtags:
            ...
            if pc is not None: #bitcoin=yes & contact"email from tags.py
                continue 
            if dc is not None: #should ignore double colon tags
                continue
            ...
            if element.tag == "way" and refname == "ref": #if it is a way and references nodes
                ...
                if kvalue == "" or vvalue == "": #make sure no empty tags get copied over 
                    continue
                node[kvalue] = vvalue
                print kvalue, vvalue, refvalue, refname 
        data.append(node)
        counter += 1
        print counter #gives an idea of the progress of the task
    else:
        skip_counter += 1
        print "skipped " + str(skip_counter) + " elements" 
        pass
    element.clear()
